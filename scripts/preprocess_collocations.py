...
def get_words(filename, max_rank):
    with open(filename) as f:
        freqs = json.load(f)
        words = [tuple[0] for tuple in sorted(freqs.items(), key=lambda kvp: kvp[1], reverse=True)]
        return words[0:max_rank]
    ...
    with open(args.filename) as f:
        for line in f:
            if should_block(line.strip(), blocklist):
                continue
            tokens = tokenize(
                line.strip(), args.target_language, args.target_ignore_case)
