...
def main():
    ...
    with open(args.target_sentences_filename) as target_sentences:
        with open(args.base_sentences_filename) as base_sentences:
            for line in target_sentences:
                ...
                no_punctuation = remove_punctuation(target.lower())
                if(len(words) == 0):
                    continue
                if(no_punctuation in seen):
                    continue
                else:
                    seen.add(no_punctuation)
    ...
    if args.top_level_only:
        ...
        for key in trie:
            with open(f"{args.output_dir.rstrip('/')}/{key.replace('/','')}.json", 'w') as f:
